...
kubectl apply -f k8s
kubectl set image deployments/server-deployement server=ashanb/multi-server:$SHA

kubectl set image deployments/client-deployement client=ashanb/multi-client:$SHA

kubectl set image deployments/worker-deployement worker=ashanb/mulit-worker:$SHA
